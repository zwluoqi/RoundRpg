...
//		if (go != null && !UGUITools.GetActive(go)) go = null;
//
//		// Try to find a panel
//		UIPanel p = (go != null) ? UGUITools.FindInParents<UIPanel>(go) : null;
//
//		// No selection? Try to find the root automatically
//		if (p == null)
//		{
//			UIPanel[] panels = UGUITools.FindActive<UIPanel>();
...
//	}
	...
	static public bool MakeTextureReadable (string path, bool force)
	{
		...
		if (force || !settings.readable || settings.npotScale != TextureImporterNPOTScale.None || settings.alphaIsTransparency)
		{
			...
			if (UGUISettings.trueColorAtlas) settings.textureFormat = TextureImporterFormat.AutomaticTruecolor;
	...
	static bool MakeTextureAnAtlas (string path, bool force, bool alphaTransparency)
	{
			...
			settings.npotScale != TextureImporterNPOTScale.ToNearest)
		{
			...
			if (UGUISettings.trueColorAtlas)
			{
...
//	static public string GetSaveableTexturePath (UGUIAtlas atlas)
//	{
...
//		{
//			path = AssetDatabase.GetAssetPath(atlas.mainTexture.GetInstanceID());
//
//			if (!string.IsNullOrEmpty(path))
//			{
...
//	}
	...
	static public void DrawSprite (Texture2D tex, Rect rect, Sprite sprite, Color color)
	{
	...
	static public void DrawSprite (Texture2D tex, Rect drawRect, Sprite sprite, Color color, Material mat)
	{
		if (!tex || sprite == null) return;
		DrawSprite(tex, drawRect, color, mat, sprite.rect.x, sprite.rect.y, sprite.rect.width, sprite.rect.height,
			sprite.rect.xMin, sprite.rect.yMax, sprite.rect.xMax, sprite.rect.yMin);
	}
	...
	{
		UGUIEditorTools.DrawSprite(tex, drawRect, color, null,
	...
	static public void DrawSprite (Texture2D tex, Rect drawRect, Color color, Material mat,
		float x, float y, float width, float height, float borderLeft, float borderBottom, float borderRight, float borderTop)
	{
		if (!tex) return;
		...
		if (Selection.activeGameObject == null || Selection.gameObjects.Length == 1)
		{
			...
			{
				tex = UGUIEditorTools.contrastTexture;
				GUI.color = Color.white;

				if (borderLeft > 0)
				{
					float x0 = (float)borderLeft / width * outerRect.width - 1;
					UGUIEditorTools.DrawTiledTexture(new Rect(x0, 0f, 1f, outerRect.height), tex);
				}

				if (borderRight > 0)
				{
					float x1 = (float)(width - borderRight) / width * outerRect.width - 1;
					UGUIEditorTools.DrawTiledTexture(new Rect(x1, 0f, 1f, outerRect.height), tex);
				}

				if (borderBottom > 0)
				{
					float y0 = (float)(height - borderBottom) / height * outerRect.height - 1;
					UGUIEditorTools.DrawTiledTexture(new Rect(0f, y0, outerRect.width, 1f), tex);
				}

				if (borderTop > 0)
				{
					float y1 = (float)borderTop / height * outerRect.height - 1;
					UGUIEditorTools.DrawTiledTexture(new Rect(0f, y1, outerRect.width, 1f), tex);
				}
			}
			GUI.EndGroup();
	...
	static public void DrawSpriteField (string label, UGUIAtlas atlas, string spriteName, SpriteSelector.Callback callback, params GUILayoutOption[] options)
	{
		GUILayout.BeginHorizontal();
	...
	static public void DrawPaddedSpriteField (string label, UGUIAtlas atlas, string spriteName, SpriteSelector.Callback callback, params GUILayoutOption[] options)
	{
		GUILayout.BeginHorizontal();
		GUILayout.Label(label, GUILayout.Width(76f));
	...
	static public void DrawSpriteField (string label, string caption, UGUIAtlas atlas, string spriteName, SpriteSelector.Callback callback, params GUILayoutOption[] options)
	{
		GUILayout.BeginHorizontal();
		GUILayout.Label(label, GUILayout.Width(76f));
	...
	static public bool DrawSpriteField (UGUIAtlas atlas, string spriteName, SpriteSelector.Callback callback, params GUILayoutOption[] options)
	{
		if (atlas.GetSprite(spriteName) == null)
			spriteName = "";

		if (UGUIEditorTools.DrawPrefixButton(spriteName, options))
		{
			UGUISettings.atlas = atlas;
			UGUISettings.selectedSprite = spriteName;
			SpriteSelector.Show(callback);
	...
	static public void DrawSpriteField (string label, SerializedObject ob, SerializedProperty atlas, SerializedProperty sprite, float width, bool padded, bool removable, params GUILayoutOption[] options)
	{
		if (atlas != null && atlas.objectReferenceValue != null)
		{
			...
			else
			{
				...
				{
					if (GUILayout.Button(spriteName, "MiniPullDown", options))
						SpriteSelector.Show(ob, sprite, atlas.objectReferenceValue as UGUIAtlas);
				}
	...
	static public void DrawAdvancedSpriteField (UGUIAtlas atlas, string spriteName, SpriteSelector.Callback callback, bool editable,
...
//							sprite.name = newName;
//
//							List<Image> sprites = FindAll<Image>();
//
//							for (int i = 0; i < sprites.Count; ++i)
//							{
//								Image sp = sprites[i];
//
//								if ( sp.sprite.name == spriteName)
//								{
//									UGUIEditorTools.RegisterUndo("Edit Sprite Name", sp);
//									sp.sprite.name = newName;
//								}
//							}
//
//							mLastSprite = newName;
...
//				}
	...
	{
		if (UGUIAtlasInspector.instance != null)
			UGUIAtlasInspector.instance.Repaint();

//		if (UGUIAtlasMaker.instance != null)
//			UGUIAtlasMaker.instance.Repaint();
	...
	static public void SelectSprite (string spriteName)
	{
		...
		{
			UGUISettings.selectedSprite = spriteName;
			UGUIEditorTools.Select(UGUISettings.atlas.gameObject);
			RepaintSprites();
		}
	}
	...
	static public void SelectSprite (UGUIAtlas atlas, string spriteName)
	{
		if (atlas != null)
		{
			UGUISettings.atlas = atlas;
			UGUISettings.selectedSprite = spriteName;
			UGUIEditorTools.Select(atlas.gameObject);
			RepaintSprites();
		}
	}
...
//	}
	...
	static public void FixUniform (GameObject go)
	{
		...
		{
			if (!UGUIEditorTools.IsUniform(t.localScale))
			{
				UGUIEditorTools.RegisterUndo("Uniform scaling fix", t);
	...
	static public bool DrawHeader (string text) { return DrawHeader(text, text, false, UGUISettings.minimalisticLook); }

	/// <summary>
	/// Draw a distinctly different looking header label
	/// </summary>

	static public bool DrawHeader (string text, string key) { return DrawHeader(text, key, false, UGUISettings.minimalisticLook); }
	...
	static public void BeginContents () { BeginContents(UGUISettings.minimalisticLook); }
	...
	static public float SceneViewDistanceToRectangle (Vector3[] worldPoints, Vector2 mousePos)
	{
		...
		return UGUIMath.DistanceToRectangle(screenPoints, mousePos);
	}

	/// <summary>
	/// Raycast into the specified panel, returning a list of widgets.
	/// Just like UGUIMath.Raycast, but doesn't rely on having a camera.
	/// </summary>
...
//		#endif
//			UIWidget.showHandlesWithMoveTool && !UGUISettings.showTransformHandles;
//		#endif
//	}
//
	/// <summary>
...
//	static public void UpgradeTexturesToSprites (UGUIAtlas atlas)
//	{
//		if (atlas == null) return;
//		List<RawImage> uits = FindAll<RawImage>();
//
//		if (uits.Count > 0)
//		{
//			UIWidget selectedTex = (UIWidgetInspector.instance != null && UIWidgetInspector.instance.target != null) ?
//				UIWidgetInspector.instance.target as RawImage : null;
//
...
//				{
//					Sprite atlasSprite = atlas.GetSprite(uiTexture.mainTexture.name);
//
//					if (atlasSprite != null)
//					{
//						SerializedObject ob = ReplaceClass(uiTexture, spriteID);
//						ob.FindProperty("mSpriteName").stringValue = uiTexture.mainTexture.name;
//						ob.FindProperty("mAtlas").objectReferenceValue = UGUISettings.atlas;
//						ob.ApplyModifiedProperties();
//					}
//				}
//			}
...
//	{
//		List<UIWidget> widgets = UGUIEditorTools.SceneViewRaycast(screenPos);
...
//			}
//
//			UIWidgetContainer wc = UGUITools.FindInParents<UIWidgetContainer>(w.cachedGameObject);
//
...
//	}
	...
	static public Color32[] AddBorder (Color32[] colors, int width, int height)
	{
		...
		{
			int y1 = UGUIMath.ClampIndex(y2 - 1, height);

			for (int x2 = 0; x2 < w2; ++x2)
			{
				int x1 = UGUIMath.ClampIndex(x2 - 1, width);
				int i2 = x2 + y2 * w2;
	...
	{
		if (!UGUISettings.minimalisticLook)
			GUILayout.Space(18f);
	}
